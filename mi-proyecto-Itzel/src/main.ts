 ...
 */
const getCarritos = async (): Promise<void> => {
  ...
  // Usamos 'as Carrito[]' para decirle a TS que confíe en nuestra interfaz.
  const listaCarritos: Carrito[] = data as Carrito[];

  // Mostramos el resultado final en la consola del navegador
  console.log("✅ Lista de carritos recibida:");
  console.table(listaCarritos); 
...
const runLaboratory = async () => {
  ...
  await createNewPost();  
  await fetchCommentsByPost(POST_ID_TO_SEARCH);
  await getCarritos(); // DESCOMENTAR ESTA LINEA PARA PROBAR LA CONEXIÓN CON SUPABASE  
  //await getCarritos();                
